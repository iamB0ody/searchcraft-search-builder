...
export class GulfTalentPlatformAdapter implements PlatformAdapter {
  readonly id = 'gulftalent';
  readonly label = 'GulfTalent';
  readonly description = 'GulfTalent job search for MENA region';
  readonly notes = [
    'GulfTalent supports keyword search with limited Boolean',
    'Best for professional jobs in Gulf countries',
    'Queries are simplified for compatibility'
  ] as const;
  readonly icon = 'briefcase-outline';
  readonly supportedSearchTypes = ['jobs'] as const;

  private readonly BASE_URL = 'https://www.gulftalent.com/jobs/search';
  ...
  buildQuery(payload: QueryPayload): PlatformQueryResult {
    ...
    warnings.push('GulfTalent has limited Boolean support; complex queries are simplified.');

    const query = simplifyBoolean(payload);
    const operatorCount = countSimplifiedOperators(query);

    if (operatorCount > 5) {
      warnings.push('Consider simplifying your search for better GulfTalent results.');
    }
  ...
  validate(payload: QueryPayload, booleanQuery: string): ValidationResult {
    ...
    warnings.push('GulfTalent works best with simpler keyword searches.');
