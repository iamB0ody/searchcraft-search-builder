...
export class GooglePostsLinkedInPlatformAdapter implements PlatformAdapter {
  readonly id = 'google-posts-linkedin';
  readonly label = 'Google \u2192 LinkedIn';
  readonly description = 'Use Google to search LinkedIn posts with full boolean support';
  readonly notes = [
    'Full boolean support (AND, OR, NOT, parentheses)',
    'Great for finding older posts not in LinkedIn\'s recent feed',
    'Indexes public LinkedIn posts and articles'
  ] as const;
  ...
  private readonly SITE_PREFIX = 'site:linkedin.com/posts';
  ...
  getCapabilities(): PlatformCapabilities {
    return {
      ...
      region: 'global',
      supportsHashtags: false, // Google doesn't treat # specially
  ...
  validate(payload: QueryPayload, booleanQuery: string): ValidationResult {
    ...
    if (!booleanQuery.trim()) {
      warnings.push('Empty query will search all LinkedIn posts (may return many results).');
    }
